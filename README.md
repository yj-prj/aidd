...
매일 오전 9시 / 오후 4시 KST, 해외·국내 **12개 소스**에서 AI 개발도구 트렌드를 수집하고,
**중복 제거**(소스 간 + 과거 14일 이력)를 거친 뒤 Gemini가 필터링·번역·분석을 **단 1회 호출**로
처리해 Slack으로 발송합니다.
...
```
1. seen 로드  지난 14일 발송 이력 (GitHub Actions 캐시)
      ↓
2. 수집       해외 8 + 국내 4, 총 12개 소스에서 뉴스 가져오기
      ↓
3. 중복 제거  URL 정규화(utm_*, fbclid 제거) + 제목 정규화로
              cross-source 중복 + 과거 이력 중복 제거
      ↓
4. Gemini     필터링 + 번역 + 분석을 단 1번 호출로 처리
      ↓
5. Slack      분석 → 개별 뉴스 + 링크 → 한 줄 요약 순서로 발송
      ↓
6. seen 저장  발송된 항목을 seen.json에 기록 → 캐시 저장
```
...
| 👽 Reddit r/LocalLLaMA | RSS — Top 24h | 최대 2개 |
| 🧪 Reddit r/MachineLearning | RSS — Top 24h | 최대 2개 |
| 🤗 Hugging Face Blog | RSS — 최신 20개 | 최대 2개 |
| 🦞 Lobsters (AI 태그) | RSS — 최신 25개 | 최대 2개 |
| 🟪 DEV.to (AI 태그) | RSS — 최신 30개 | 최대 2개 |
...
| 🟨 카카오 테크 | RSS — 최신 20개 | 최대 1개 |

---

## 중복 제거 (v5 신규)

같은 글이 매일 반복 발송되거나, 같은 글이 여러 소스에 동시에 떠서 두 번 발송되는 문제를 해결합니다.

**동작 방식**

1. **URL 정규화** — `utm_source`, `utm_medium`, `fbclid`, `gclid`, `mc_cid` 등 트래킹 파라미터,
   `www.` 접두어, trailing slash, fragment(`#…`)를 제거한 뒤 비교
2. **제목 정규화** — 소문자화 + 기호 제거 후 공백 단일화 → 같은 제목의 다른 URL도 잡아냄
3. **이력 보관** — 발송된 항목을 `seen.json`에 ISO 타임스탬프로 저장
4. **TTL** — `SEEN_TTL_DAYS`(기본 14일) 지난 항목은 자동 만료
5. **영속성** — `actions/cache`로 워크플로우 실행 간 `seen.json` 유지 (커밋 노이즈 없음)

**환경 변수로 조절**

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `SEEN_FILE` | `seen.json` | 이력 파일 경로 |
| `SEEN_TTL_DAYS` | `14` | 며칠 동안 같은 항목을 재발송 안 할지 |
...
**② 번역 + 요약**
필터링 통과한 해외 소스(HN, Product Hunt, GitHub, Reddit, HF, Lobsters, DEV.to)를 한국어로 번역합니다.
